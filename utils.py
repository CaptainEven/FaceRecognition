...
def select_car_triplet(dir, labels, label2img):
    ...
    positive_id = get_negative_id(len(label2img[labels[anchor_cls]]), anchor_id)
    ...
    positive_path = os.path.join(dir, label2img[labels[anchor_cls]][positive_id])
    negative_path = os.path.join(dir, label2img[labels[negative_cls]][negative_id])
...
def process_cars(meta_path, annot_path, orig_train_dir, limit=40):
    ...
    for f in files:
        ...
        if len(cv2.split(img)) == 1:
            ...
            print('%s is converted to %d channels.'%(f, len(cv2.split(img_cvt))))
